...
vr_data$Monitor_scenario_score <- replace(vr_data$Monitor_scenario_score, vr_data$Monitor_scenario_score == 0, NA)
vr_data$Monitor_scenario_score <- replace(vr_data$Monitor_scenario_score, vr_data$Monitor_scenario_score == 1, 5)
vr_data$Monitor_scenario_score <- replace(vr_data$Monitor_scenario_score, vr_data$Monitor_scenario_score == 4, 1)
vr_data$Monitor_scenario_score <- replace(vr_data$Monitor_scenario_score, vr_data$Monitor_scenario_score == 5, 4)


table(vr_data$Monitor_scenario_score)
```
...
```{r}
grades <- vr_data[, 5]
confidence <- vr_data[, 13]

# Combine the grade and confidence variables into a data frame
data_clean <- data.frame(grades, confidence)
data_clean <- na.omit(data_clean)

count_data <- data_clean %>%
  group_by(Monitor_scenario_score, Monitor_confidence) %>%
  summarise(Count = n())

ggplot(count_data, aes(x = Monitor_scenario_score, y = Monitor_confidence, fill = Count)) +
  geom_tile() +
  scale_fill_gradient(low = "white", high = "blue") +
  labs(x = "נא לסמן את הבדיקות אשר ביצעת בתרחיש במוניטור שבנטן של מדא (במידה וביצעת את תרחיש המוניטור)", y = "באיזה מידה אתה יודע להשתמש במוניטור", fill = "Count")
...
# Trying distance correlation function between the "can_the_simulator_improve_your_ability_to_treat_similar_cases" as y and the rest of the columns in the eval_dataset as x
...
distance_correlation <- function(x, y) {
  ...
  # Calculate mean-centered distance matrices
  A <- dist_x - rowMeans(dist_x) %*% t(rep(1, n))
  B <- dist_y - rowMeans(dist_y) %*% t(rep(1, n))
  
  # Calculate products of mean-centered matrices
  C <- A %*% t(A)
  D <- B %*% t(B)
  ...
  cov_xy <- sqrt(sum(C * D)) / (n^2)
  
  # Calculate distance correlation
  dcov <- sqrt(cov_xy)
  dcor <- dcov / sqrt(sqrt(var_x) * sqrt(var_y))
  
  return(dcor)
}

# Select the dependent variable and independent variables
dependent_variable <- evaluation_dataset$can_the_simulator_improve_your_ability_to_treat_similar_cases
independent_variables <- evaluation_dataset[, -ncol(evaluation_dataset)]

# Calculate distance correlation
correlation <- distance_correlation(independent_variables, dependent_variable)

correlation


```
### We can see that we got a poor distance correlation
